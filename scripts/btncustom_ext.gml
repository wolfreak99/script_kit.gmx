///btncustom_ext(x,y,width,height,string)
/***************************************************
  ...
  Variables needed in draw (BEFORE buttons):
    btn_int = 0;
  
  Use script like:
    if (room_width / 2, room_height / 2, 50, 10, "Reset")
        do_reset();
  
 ***************************************************/
//INITIALIZE
var xx, yy, text, text_width, text_height, rect_x, rect_y, rect_x2, rect_y2,
        mousex, mousey, btn_state, btn_alpha, btn_color, txt_alpha, txt_color;

xx = argument0;
yy = argument1;
text = argument4;

text_width = max(argument2, string_width(text) + 2);
text_height = max(argument3, string_height(text) + 2);

rect_x = xx - text_width;
rect_y = yy - text_height;
rect_x2 = xx + text_width;
rect_y2 = yy + text_height;

mousex = mouse_x; //if you want to use a different mouse coordinate, switch this.
mousey = mouse_y;


//To add your own colors, comment out or remove line 40 and 41, and then move lines 43 - 54 outside of the script (before it's called).
var btn_alpha_off, btn_color_off, btn_alpha_hover, btn_color_hover, btn_alpha_press, btn_color_press,
        txt_alpha_off, txt_color_off, txt_alpha_hover, txt_color_hover, txt_alpha_press, txt_color_press;
...
switch btn_state
{
    ...
    case BTN_PRESS:
        ...
        break;
    default:
        show_exception_unknown_variable("btn_state", btn_state, "btncustom_ext", true);
        break;
}
//DRAW
...
//increase integer for next button.
btn_int += 1;
